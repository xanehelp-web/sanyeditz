    ...
    <section id="about" className="py-24 px-6 bg-dark">
      <div className="max-w-7xl mx-auto grid md:grid-cols-2 gap-20 items-center">
        ...
        <motion.div
          ...
          whileInView={{ opacity: 1, x: 0 }}
          viewport={{ once: true }}
          className="bg-surface p-6 minecraft-border"
        >
          <h3 className="text-xl font-display mb-6 text-foreground">Technical Skills</h3>
          <div className="grid grid-cols-2 gap-4">
            {SKILLS.map((skill) => (
              <div key={skill.name} className="flex flex-col items-center gap-3 p-4 minecraft-border hover:border-primary transition-colors group">
                <div className="text-primary group-hover:scale-110 transition-transform">
                  {getIcon(skill.icon)}
                </div>
                <span className="text-[10px] font-display uppercase tracking-normal text-center text-slate-400 group-hover:text-foreground transition-colors">
                  {skill.name}
                </span>
              </div>
            ))}
